...
def get_criterion(model):
    ...
    return [criterion_dict[model],criterion_dict['CenterLossWithLanguage'],criterion_dict['InstanceContrastiveLoss']]
...
def train_one_epoch(model, optimizer, data_loader, lr_scheduler, epoch, print_freq, loss_scaler, clip_grad, n_epoch):
    model.train()
    optimizer.zero_grad()
    metric_logger = utils.MetricLogger(delimiter="  ")
    ...
    for data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        total_loss = loss_dict['total_loss1'] + loss_dict['total_loss2'] + loss_dict['total_loss3']
...
def main(args, distributed):
    print('will be saved as model_best_refcoco !!!')
    ...
    print('get_world_size: ',num_tasks) 
    ...
    for epoch in range(max(0, resume_epoch+1), args.epochs):
        ...
        train_one_epoch(model, optimizer, data_loader, lr_scheduler, epoch, args.print_freq, loss_scaler, clip_grad,epoch)
        ...
        # best_dict_to_save = dict_to_save 
        utils.save_on_master(dict_to_save, os.path.join(args.output_dir,
                                                        'model_checkpoint_{}.pth'.format(args.model_id)))
