...
class VLMSDeformAttnLayer(nn.Module):
    ...
    def forward(self, vis, lang, prompts=None, lang_mask=None, vis_pos=None, vis_padding_mask=None, return_attn=False, **kwargs):
        ...
        # 添加URCE
        # _vis, l2v_attn = self.l2v_cross_attn(q=PosEncoding(vis, vis_pos), k=prompted_lang, v=prompted_lang, attention_mask=lang_mask)
        # print('l2v_attn.shape: ',l2v_attn.shape)
...
# 原代码  可用于对比学习
# class VLMSDeformAttnPixelDecoder1(BaseModule):
class VLMSDeformAttnPixelDecoder(BaseModule):
    ...
    def __init__(self,
                 in_channels=[256, 512, 1024, 2048],# 128 256 512 1024
                 lang_in_channels=768,
                 strides=[4, 8, 16, 32],
                 feat_channels=256, # 256
                 out_channels=256,  # 256
                 num_enc_layers=6,    # the layer num of SVLA
                 num_heads=8,
                 num_levels=3,
                 num_points=4,
                 im2col_step=16,
                 ...
                 mlp_expand=4,
                 with_prompts=True,
                 num_prompts=10,   # 5 Mask Prompts
                 norm_cfg=dict(type='GN', num_groups=32),
                 act_cfg=dict(type='ReLU'),
                 ):
        super().__init__()
        self.strides = strides
        self.num_input_levels = len(in_channels) # 4
        self.num_encoder_levels = num_levels     # 3
        ...
        # Sine/cosine functions occur in pairs and each set of codes requires 2 dimensions (one each for sin and cos)
        ...
        # for i in range(0,-1,-1)---->just one layers
        # output_conv[later_conv(V1) + V2]
    ...
    def forward(self, feats, lang, lang_mask, return_attn=False):
        ...
        for i in range(self.num_encoder_levels):# (0,1,2) 
            # num_input_levels = 4 num_encoder_levels = 3
            level_idx = self.num_input_levels - i - 1  # 3-->2-->1   V4 --> V3 --> V2
            # feats:[V1,V2,V3,V4]
            feat = feats[level_idx]
            feat_projected = self.input_convs[i](feat) # C--->256
            h, w = feat.shape[-2:]

            if i == 0:
                v4_feature = feat_projected

            
            # no padding-->Create False Mask
            padding_mask_resized = feat.new_zeros(
                (batch_size, ) + feat.shape[-2:], dtype=torch.bool) # [B,H,W]
            pos_embed = self.postional_encoding(padding_mask_resized)# (batch_size, feat_channels, height, width)
            level_embed = self.level_encoding.weight[i]#(feat_channels, )
            level_pos_embed = level_embed.view(1, -1, 1, 1) + pos_embed # (batch_size, feat_channels, height, width)
            ...
            padding_mask_resized = padding_mask_resized.flatten(1)# -->[B,H*W]

            encoder_input_list.append(feat_projected) #[V4,V3,V2]
        ...
        # extract V4 feature shapes:[256]
        # v4_feature = encoder_input_list[0]
        # v4_feature = v4_feature.view(8,14,14,256).permute(0,3,1,2)
        ...
        #(batch_size, ΣH_iW_i, num_encoder_levels, 2)
        reference_points = reference_points[None, :, None].repeat(
            batch_size, 1, self.num_encoder_levels, 1)
        #(batch_size, num_encoder_levels, 2)
        ...
        #i = range(4-3-1,-1,-1) = range(0,-1,-1)-->i = [0]
        for i in range(self.num_input_levels - self.num_encoder_levels - 1, -1,
                       -1):
            x = feats[i]# V1
        ...
        prompts_g = self.lang_embed(torch.mean(prompts, dim=1, keepdim=True))#[8,1,256]
        lang_cls = torch.cat([prompts_g, lang_g], dim=1) # [B, 2, C]  2quary
        ...
        return mask_pred,lang_cls[:,1,:],mask_feature # 最后特征
